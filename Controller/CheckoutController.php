...
class CheckoutController
{
    ...
    private function procesarPago()
    {
        ...
        try {
            ...
            if ($this->isAjax) {
                $this->sendJsonResponse(true, 'Pedido procesado correctamente', [
            ...
            if ($this->isAjax) {
                $this->sendJsonResponse(false, 'Error al procesar el pedido');
            } else {
                $_SESSION['mensaje'] = 'Error al procesar el pedido';
                $_SESSION['alerta'] = 'alert-danger';
                $this->redirect('CarritoController.php?action=ver');
            }
        }
    }
